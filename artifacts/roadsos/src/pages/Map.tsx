...
import { useGetNearbyServices, getGetNearbyServicesQueryKey } from "@workspace/api-client-react";
...
export default function MapPage() {
  ...
  const { data: services, isLoading } = useGetNearbyServices(
    { lat: 37.7749, lng: -122.4194, type: filter === "ALL" ? undefined : filter.toLowerCase() },
    { query: { queryKey: getGetNearbyServicesQueryKey({ lat: 37.7749, lng: -122.4194, type: filter === "ALL" ? undefined : filter.toLowerCase() }) } }
  );

  // Fallback dummy data if API is not connected or returns empty
  const dummyServices = [
    { id: "1", name: "City General Hospital", type: "HOSPITAL", distance: 1.2, eta: 4, available: true, phone: "555-0101" },
    { id: "2", name: "Central Police Precinct", type: "POLICE", distance: 2.5, eta: 8, available: true, phone: "555-0102" },
    { id: "3", name: "Rapid Towing Co.", type: "TOWING", distance: 0.8, eta: 3, available: true, phone: "555-0103" },
    { id: "4", name: "Mike's Puncture Repair", type: "PUNCTURE", distance: 3.1, eta: 12, available: false, phone: "555-0104" },
  ];

  const displayServices = services?.length ? services : dummyServices.filter(s => filter === "ALL" || s.type === filter);
          ...
          {displayServices.map((service, i) => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="font-bold text-white tracking-wide">{service.name}</h3>
                  <p className="text-xs text-muted-foreground uppercase">{service.type}</p>
                </div>
                <div className="text-right">
                  <div className="text-accent font-orbitron font-bold">{service.distance} KM</div>
                  <div className="text-xs text-white/50">{service.eta} min ETA</div>
                </div>
              </div>
              
              <div className="flex gap-2 mt-4">
                <Button variant="outline" size="sm" className="flex-1 border-white/10 hover:bg-white/5 h-10">
                  <Phone className="w-4 h-4 mr-2" /> CALL
                </Button>
                <Button size="sm" className="flex-1 bg-accent/20 text-accent hover:bg-accent/30 border border-accent/30 h-10">
