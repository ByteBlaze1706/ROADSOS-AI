...
import { FileText, Plus, Calendar, MapPin, Search } from "lucide-react";
...
export default function Incidents() {
  ...
  const { data: incidents, refetch } = useGetIncidents({
  ...
  const getSeverityBadge = (sev) => {
    switch (sev.toUpperCase()) {
      ...
      case "CRITICAL":
        return (
          <span className="bg-red-500/20 text-red-500 border border-red-500/30 px-2 py-0.5 rounded text-[10px] font-bold tracking-wider">
            CRITICAL
          </span>
        );
      case "MEDIUM":
        return (
          <span className="bg-yellow-500/20 text-yellow-500 border border-yellow-500/30 px-2 py-0.5 rounded text-[10px] font-bold tracking-wider">
            MEDIUM
          </span>
        );
      default:
        return (
          <span className="bg-green-500/20 text-green-500 border border-green-500/30 px-2 py-0.5 rounded text-[10px] font-bold tracking-wider">
            LOW
          </span>
        );
    }
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs text-muted-foreground">
                  LOG-{inc.id.toString().padStart(4, "0")}
                </span>
              </div>
            ...
            <div className="flex flex-col gap-1 text-sm text-white/60">
              <div className="flex items-center gap-2">
                ...
                <span>
                  {new Date(inc.date || inc.createdAt).toLocaleDateString()}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <MapPin className="w-3.5 h-3.5" />
                <span>{inc.location || "GPS Coordinates Logged"}</span>
              </div>
            </div>

            <div className="mt-4 pt-3 border-t border-white/5 flex justify-between items-center">
              <span
                className={`text-[10px] uppercase tracking-widest font-bold ${inc.status === "RESOLVED" ? "text-muted-foreground" : "text-accent"}`}
              >
                STATUS: {inc.status}
              </span>
