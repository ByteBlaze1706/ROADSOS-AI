...
import {
  useTriggerSos,
  useGetEmergencyStats,
  getGetEmergencyStatsQueryKey,
...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 space-y-6">
        ...
        <div className="relative">
          <AnimatePresence>
            {countdown !== null && (
              ...
              >
                <span className="text-5xl font-orbitron font-bold text-primary neon-text-red">
                  {countdown}
                </span>
                <span className="text-xs tracking-widest text-muted-foreground mt-2 uppercase">
                  Dispatching
                </span>
                <Button
                  variant="outline"
          ...
          >
            ...
            <span className="font-orbitron text-5xl font-bold text-white tracking-widest shadow-black drop-shadow-lg z-10">
              SOS
            </span>
            <span className="text-white/80 text-sm mt-1 tracking-widest uppercase font-bold z-10">
              Hold to activate
            </span>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="font-orbitron font-bold text-xl text-green-500">
              {stats?.gpsStatus || "STRONG"}
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        <h3 className="text-sm font-orbitron text-muted-foreground uppercase tracking-wider mb-3">
          Rapid Deployment
        </h3>
        ...
        <DialogContent className="bg-card border-white/10 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-orbitron text-xl uppercase tracking-wider text-accent">
              Nearby {quickActions.find((a) => a.id === selectedService)?.label}
              s
            </DialogTitle>
          ...
          <div className="space-y-3 mt-4">
            {dummyServices
              .filter(
                (s) => s.type === selectedService || selectedService === null,
              )
              .map((service, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center p-3 rounded-lg bg-black/40 border border-white/5 hover:border-accent/50 transition-colors"
                >
                  <div>
                    <h4 className="font-bold text-white">{service.name}</h4>
                    <p className="text-xs text-muted-foreground">
                      {service.distance} KM • {service.eta} MIN ETA
                    </p>
                  </div>
