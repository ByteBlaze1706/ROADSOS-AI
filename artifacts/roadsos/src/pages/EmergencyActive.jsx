...
import { AlertOctagon, Phone, MapPin, XOctagon } from "lucide-react";
import { useAuth } from "@/lib/auth";
import { Button } from "@/components/ui/button";
import {
  useCancelSos,
  useGetActiveSos,
  getGetActiveSosQueryKey,
...
export default function EmergencyActive() {
  ...
  const { data: activeSosData } = useGetActiveSos({
    query: { queryKey: getGetActiveSosQueryKey() },
  });
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(
      () =>
        setStages((s) =>
          s.map((st) => (st.id === 5 ? { ...st, status: "completed" } : st)),
        ),
      9000,
    );
    return () => {
  ...
  const handleCancel = () => {
    if (activeSosData?.alert?.id) {
      cancelSosMutation.mutate(
        { id: activeSosData.alert.id },
        {
          onSuccess: () => {
            setActiveSos(false);
    ...
    <div className="min-h-screen bg-[#1a0000] text-white flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 flex-1 flex flex-col p-4">
        ...
        <div className="flex flex-col items-center justify-center my-8">
          <span className="text-sm text-red-400 uppercase tracking-widest mb-2 font-bold">
            Estimated Arrival
          </span>
            ...
            {stages.map((stage, i) => (
              <div key={stage.id} className="flex items-start gap-4">
                <div className="relative flex flex-col items-center">
                  ...
                  {i < stages.length - 1 && (
                    <div
                      className={`w-0.5 h-10 ${stage.status === "completed" ? "bg-red-500/50" : "bg-white/10"}`}
                    />
                  )}
                </div>
                <div
                ...
                >
                  <p className="font-orbitron tracking-wider text-sm">
                    {stage.label}
                  </p>
                  {stage.status === "active" && (
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          ...
          <Button className="h-14 bg-red-600 hover:bg-red-700 text-white neon-box-red font-bold">
